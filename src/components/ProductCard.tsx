...
export function ProductCard({
  ...
  return (
    ...
    >
      <div
        className="relative overflow-hidden rounded-[var(--radius)] bg-surface-2"
        style={{ aspectRatio: "1" }}
      >
        ...
        {p.isBestseller && !p.ratingSynthetic && (
          <span className="absolute left-2 top-2 rounded-[3px] bg-white px-2 py-1 text-[11px] font-medium text-ink shadow-sm">
            Bestseller
          </span>
        )}
        <button
        ...
        <button
          type="button"
          aria-label="Add"
          className="absolute bottom-2 right-2 flex h-9 w-9 items-center justify-center rounded-full bg-white text-ink shadow-sm"
        >
      ...
      <div className="mt-2">
        <p className="flex items-baseline gap-1.5">
          ...
          <span className="text-[12px] text-muted">
            MSRP {formatPrice(p.msrp)}
          </span>
        </p>
        <p className="line-clamp-2 text-[14px] font-medium leading-[1.3] text-ink">
          {p.name}
        </p>
