...
export function DeviceShell({ children }: { children: ReactNode }) {
  ...
  const setBackend = (backend: EmbeddingBackend) => {
    ...
    if (
      ...
      router.push(
        `/onboarding/building?return=${pathname.startsWith("/search") ? "search" : "home"}`,
      );
    }
  };
  const restart = () => {
  ...
  const header = (
    <header className="top-nav">
      <Link
        ...
        aria-label="Faire Onboarding Store Scan: start over"
      >
        <span className="top-nav__mark font-serif" aria-hidden>
          F
        </span>
        <span className="font-serif">Faire Onboarding Store Scan</span>
      </Link>
      <nav className="top-nav__links">
        <label className="top-nav__select">
          <span>Retrieval</span>
          <select
            value={state.embeddingBackend}
            onChange={(e) => setBackend(e.target.value as EmbeddingBackend)}
            aria-label="Retrieval method"
          >
            {(Object.keys(EMBEDDING_BACKEND_LABEL) as EmbeddingBackend[]).map(
              (b) => (
