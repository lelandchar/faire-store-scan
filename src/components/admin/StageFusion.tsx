    ...
    >
      <div className="grid gap-6 md:grid-cols-2">
        <div>
          ...
          <p className="text-caption mt-3 max-w-[520px]">
            ...
            <>
              <ul className="space-y-2">
                {profile.categories.map((c) => (
                  <li key={c.name} className="rounded-[var(--radius)] border border-line px-3 py-2">
                    <div className="flex items-baseline justify-between gap-2 text-[12px]">
                      <span className="font-medium text-ink">{c.name}</span>
                      <Mono className="text-muted">{c.share}</Mono>
                    </div>
              ...
              <tbody>
                {ranked.slice(0, 40).map((r) => (
                  <tr key={r.product.id} className={r.score < 0 ? "opacity-50" : ""}>
                    ...
                    <td className={`${TD} min-w-[220px] text-ink-2`}>{r.reasons[0]?.text ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
